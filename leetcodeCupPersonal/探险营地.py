...
class Solution:
    def adventureCamp(self, expeditions: List[str]) -> int:
        init = set(expeditions[0].split('->'))
        res = len(expeditions)
        new = 0
        for i in range(1, len(expeditions)):
            thisTimeCamp = set(expeditions[i].split('->'))
            # print(thisTimeCamp)
            cnt = 0
            for camp in thisTimeCamp:
                if camp not in init and camp != '':
                    cnt += 1
                    init.add(camp)
            if cnt > new:
                new = cnt
...
print(Solution().adventureCamp(["xO->xO->xO","xO->BKbWDH","xO->BKbWDH","BKbWDH->mWXW","LSAYWW->LSAYWW","oAibBvPdJ","LSAYWW->u","LSAYWW->LSAYWW"]))