...
function minOperationsMaxProfit(customers: number[], boardingCost: number, runningCost: number): number {
    ...
    let curCustomer = 0;
    let curIndex = 0;
    while (curCustomer > 0 || curIndex < customers.length) {
        if (curIndex < customers.length) {
            curCustomer += customers[curIndex];
        }
        const curBoarding = Math.min(curCustomer, 4);
        curCustomer -= curBoarding;
        curProfit += curBoarding * boardingCost - runningCost;
        curIndex++;
        if (curProfit > maxProfit) {
            maxProfit = curProfit;
            maxProfitIndex = curIndex;
        }
    }
